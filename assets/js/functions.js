function atualizarClasseInput(dir) {
    value = $('input_' + dir).value;

    if (value.length === 0 || value.length === 1) {
        $('input_' + dir).className = 'chars_1 form-control form-control-sm';
    } else if (value.length == 2) {
        $('input_' + dir).className = 'chars_2 form-control form-control-sm';
    } else if (value.length == 3) {
        $('input_' + dir).className = 'chars_3 form-control form-control-sm';
    } else if (value.length == 4) {
        $('input_' + dir).className = 'chars_4 form-control form-control-sm';
    } else {
...
function gerarCodigo(option, master, tl_radius, tr_radius, br_radius, bl_radius) {
...
function atalizarTextArea() {
    ...
    var code = '';
    if (options['webkit']) code += gerarCodigo('webkit', master_radius, tl_radius, tr_radius, br_radius, bl_radius) + "---\\webkit--- \n";
    if (options['gecko']) code += gerarCodigo('gecko', master_radius, tl_radius, tr_radius, br_radius, bl_radius) + "---\\gecko--- \n";
    if (options['css3']) code += gerarCodigo('css3', master_radius, tl_radius, tr_radius, br_radius, bl_radius) + "---\\css3--- \n";
...
function observarKeydownInput(e, dir) {
...
function observe_option(opt, checked) {
    atualizarOpcoes();
    if (master_radius) atalizarTextArea();
}
...
new Form.Element.Observer('opt_css3', 0.15, function (el) {
    observe_option('css3', el.checked)
});
new Form.Element.Observer('opt_webkit', 0.15, function (el) {
    observe_option('webkit', el.checked)
});
new Form.Element.Observer('opt_gecko', 0.15, function (el) {
    observe_option('gecko', el.checked)
});
